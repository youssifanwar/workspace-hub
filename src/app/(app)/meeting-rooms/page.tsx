...
export default async function MeetingRoomsPage() {
  ...
  return <div className="space-y-8"><div><h1 className="text-3xl font-bold text-slate-900">Meeting Rooms</h1><p className="text-slate-500 mt-1">Book meeting rooms by the hour. Pricing is based on the room and number of people.{canManage(user.role) ? "" : " Ask a manager to update room pricing."}</p></div><ActiveMeetingRooms currency={currency} /><RoomsGrid rooms={roomsWithPricing} currency={currency} canEditRate={canManage(user.role)} /></div>;
}